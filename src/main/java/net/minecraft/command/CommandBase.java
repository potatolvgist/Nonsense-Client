...
public abstract class CommandBase implements ICommand
{
    ...
    public static BlockPos parseBlockPos(ICommandSender sender, String[] args, int startIndex, boolean centerBlock) throws NumberInvalidException
    {
        ...
        return new BlockPos(parseDouble(blockpos.getX(), args[startIndex], -30000000, 30000000, centerBlock), parseDouble(blockpos.getY(), args[startIndex + 1], 0, 256, false), parseDouble((double)blockpos.getZ(), args[startIndex + 2], -30000000, 30000000, centerBlock));
    }
    ...
    public static Entity getEntity(ICommandSender sender, String token) throws EntityNotFoundException
    {
        return getEntity(sender, token, Entity.class);
    }

    @SuppressWarnings("unchecked")
    public static <T extends Entity> T getEntity(ICommandSender commandSender, String token, Class <? extends T > targetClass) throws EntityNotFoundException
    {
        Entity entity = PlayerSelector.matchOneEntity(commandSender, token, targetClass);
        MinecraftServer minecraftserver = MinecraftServer.getServer();

        if (entity == null)
        {
            entity = minecraftserver.getConfigurationManager().getPlayerByUsername(token);
        }
