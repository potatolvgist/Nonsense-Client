...
public class Timer
{
    ...
    public void updateTimer()
    {
        ...
        long timePassed = now - this.lastSyncSysClock;
        long nanoSync = System.nanoTime() / 1000000L;
        double d0 = (double)nanoSync / 1000.0;

        if (timePassed <= 1000L && timePassed >= 0L)
        {
            this.counter += timePassed;

            if (this.counter > 1000L)
            {
                long l = nanoSync - this.lastSyncHRClock;
                double d1 = (double)this.counter / (double)l;
                this.timeSyncAdjustment += (d1 - this.timeSyncAdjustment) * 0.2;
                this.lastSyncHRClock = nanoSync;
                this.counter = 0L;
            }
