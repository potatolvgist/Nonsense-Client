...
public class Gui
{
    ...
    public static void drawRect(float left, float top, float right, float bottom, int color) {
        drawRect(Math.round(left), Math.round(top), Math.round(right), Math.round(bottom), color);
    }
    ...
    public static void drawModalRectWithCustomSizedTexture(int x, int y, float u, float v, int width, int height, float textureWidth, float textureHeight)
    {
        ...
        renderer.pos(x, y + height, 0.0D).tex(u * f, (v + (float)height) * f1).endVertex();
        renderer.pos(x + width, y + height, 0.0D).tex((u + (float)width) * f, (v + (float)height) * f1).endVertex();
        renderer.pos(x + width, y, 0.0D).tex((u + (float)width) * f, v * f1).endVertex();
