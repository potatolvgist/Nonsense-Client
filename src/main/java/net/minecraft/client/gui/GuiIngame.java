...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        Hud.drawComponents(scaledResolution, partialTicks);
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes, int minY) {
        ...
        List<Score> filteredScores = scores.stream().filter(score -> score.getPlayerName() != null && !score.getPlayerName().startsWith("#")).collect(Collectors.toList());
        ...
        for (Score score1 : scores) {
            ...
            drawRect(l1 - 2, k, l, k + this.getFontRenderer().FONT_HEIGHT, 0x50000000);
            ...
            if (j == scores.size()) {
                ...
                drawRect(l1 - 2, k - this.getFontRenderer().FONT_HEIGHT - 1, l, k - 1, 0x60000000);
                drawRect(l1 - 2, k - 1, l, k, 0x50000000);
                this.getFontRenderer().drawString(s3, l1 + objectiveWidth / 2 - this.getFontRenderer().getStringWidth(s3) / 2, k - this.getFontRenderer().FONT_HEIGHT, 0x20ffffff);
            }
        }
    }
    ...
    private void renderPumpkinOverlay(ScaledResolution scaledRes) {
        ...
        if (Nonsense.module(NoRender.class).overlay()) {
            return;
        }
    ...
    private void renderPortal(float timeInPortal, ScaledResolution scaledRes) {
        ...
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        worldrenderer.begin(GL_QUADS, DefaultVertexFormats.POSITION_TEX);
        worldrenderer.pos(0.0D, scaledRes.getScaledHeight(), -90.0D).tex(minU, maxV).endVertex();
        worldrenderer.pos(scaledRes.getScaledWidth(), scaledRes.getScaledHeight(), -90.0D).tex(maxU, maxV).endVertex();
        worldrenderer.pos(scaledRes.getScaledWidth(), 0.0D, -90.0D).tex(maxU, minV).endVertex();
        worldrenderer.pos(0.0D, 0.0D, -90.0D).tex(minU, minV).endVertex();
        tessellator.draw();
        GlStateManager.depthMask(true);
        GlStateManager.enableDepth();
        GlStateManager.enableAlpha();
